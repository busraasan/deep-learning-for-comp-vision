...
def compute_distances_one_loop(x_train, x_test):
  ...
  #vectorize the input matrices as column vectors
  x_train = x_train.reshape(num_train, -1)
  x_test = x_test.reshape(num_test, -1)
  # (x_train[0] - x_test) has the shape (100,768) and each entry is x_train[0] -
  # x_test[0], x_train[0] - x_test[1] ...
  # then using dim=1 we add individual difference for each feature and obtain a
  # 100x1 matrix
  for i in range(num_train):
...
def predict_labels(dists, y_train, k=1):
  ...
  ##############################################################################
  # Replace "pass" statement with your code
  for column in range(dists.shape[1]):
    closest = torch.topk(dists[:, column], k, largest=False)
    labels = y_train[closest.indices]
    elements, counts = torch.unique(labels, return_counts=True)
    index_of_max = torch.argmax(counts)
    y_pred[column] = elements[index_of_max]
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return y_pred
...
class KnnClassifier:
  ...
  def predict(self, x_test, k=1):
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    dists = compute_distances_no_loops(self.X_tr, x_test)
    y_test_pred = predict_labels(dists, x_test, k)
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
    return y_test_pred
...
def knn_cross_validate(x_train, y_train, num_folds=5, k_choices=None):
  ...
  ##############################################################################
  # Replace "pass" statement with your code
  x_train_folds = x_train.chunk(5)
  y_train_folds = y_train.chunk(5)
  ##############################################################################
  #                            END OF YOUR CODE                                #
  ##############################################################################
  ...
  ##############################################################################
  # Replace "pass" statement with your code
  x_val_set = x_train_folds[num_folds-1]
  y_val_set = y_train_folds[num_folds-1]
  for k in k_choices:
    k_to_accuracies[k] = []
    for fold in range(num_folds-1):
      x_test = x_train_folds[fold]
      y_test = y_train_folds[fold]
      train_y = [y_train_folds[j] for j in range(num_folds) if fold != j]
      train_x = [x_train_folds[j] for j in range(num_folds) if fold != j]
      nn = KnnClassifier(torch.cat(train_x), torch.cat(train_y))
      accuracy = nn.check_accuracy(x_test, y_test, k)
...
def knn_get_best_k(k_to_accuracies):
  ...
  means = []
  keys = []
  for key in range(k_to_accuracies):
    means.append(torch.mean(torch.tensor(k_to_accuracies[key])))
    keys.append[key]
  means = torch.tensor(means)
  best_index = torch.argmax(means)
  best_k = keys[best_index]
  ##############################################################################
  #                            END OF YOUR CODE                                #
  ##############################################################################
  return best_k
