...
def normalize_columns(x):
  ...
  #############################################################################
  # Replace "pass" statement with your code
  column_means = torch.sum(x, dim=0)/x.shape[0]
  std_deviation = torch.sqrt(torch.sum(((x - column_means)**2 / (x.shape[0]-1)), dim=0))
  y = (x - column_means)/std_deviation
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
  return y
